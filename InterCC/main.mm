...
_IOConnectCallScalarMethod(mach_port_t connection,
                           ...
                           uint32_t *outputCnt) {
    DEBUG_LOG_COLOR(FGGRN)
    kern_return_t result = IOConnectCallScalarMethod(connection, selector, input, inputCnt, output, outputCnt);
    qmilog("\t sel: 0x%02x\n", selector);
    qmilog("\t-> input: %d\n", inputCnt);
    if (inputCnt > 0) {
        hexdumpc((unsigned char *)input, inputCnt);
    }
    if (output != NULL) {
        qmilog("\t-> output: %d\n", *outputCnt);
        hexdumpc((unsigned char *)output, *outputCnt);
    }
    return result;
